...
/**
 * Implementation of {@link EndpointFactory} using JeroMQ-based {@code Endpoint} implementations.
 */
public class JmqEndpointFactory implements EndpointFactory {
    ...
    public Endpoint buildClientEndpoint(String hostAddress, int hostDiscoveryPort, String token,
        ...
        return new JmqClientEndpoint(hostAddress, hostSharePort, port, messageHandlerFactory);
    }

    @Override
    public HostEndpoint buildHostEndpoint(int port, MessageHandlerFactory messageHandlerFactory) {
        return new JmqHostEndpoint(port, messageHandlerFactory);
    }
    ...
    public HostEndpoint buildHostEndpoint(int port, MessageHandlerFactory messageHandlerFactory,
                                                 Authenticator
            authenticator) {
        return new JmqHostEndpoint(port, messageHandlerFactory, authenticator);
    }
}
